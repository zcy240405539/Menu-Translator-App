...
export default function HomePage({
  ...
  initialLanguage?: WebLanguageCode;
  initialText?: Catalog;
}) {
  const [menuHash, setMenuHash] = useState("");
  const [lang, setLang] = useState<WebLanguageCode>(initialLanguage);
  ...
  const text = useText(lang, initialText);
  ...
  useEffect(() => {
    ...
    queueMicrotask(() => {
      ...
      const nextLang = normalizeLanguage(
        params.get("lang")
          || pathLanguage
          || initialLanguage
          || getInitialLanguage(),
  ...
  const handleHeaderLanguageChange = (nextLang: string) => {
    ...
    if (!menuHash) {
      ...
      window.history.replaceState({}, "", localizedPublicPath("/", normalizedLang));
    } else {
      replacePageLanguage(normalizedLang);
    }
  ...
  const langQuery = languageQuery(lang);
  const historyHref = `/history${langQuery}`;
  const cartHref = `/cart${langQuery}`;
  const homeHref = localizedPublicPath("/", lang);
  ...
  return (
    <div lang={htmlLanguage(lang)} dir={lang === "ar" ? "rtl" : "ltr"} className="flex min-h-screen flex-col bg-[#fbf8f4] font-sans">
      {!showResultView && (
        <script
